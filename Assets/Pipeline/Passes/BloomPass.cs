...
namespace Graphics
{
    public class BloomPass
    {
        ...
        int bicubicUpsamplingId = Shader.PropertyToID("_BloomBicubicUpsampling");
        int thresholdId = Shader.PropertyToID("_BloomThreshold");
        int intensityId = Shader.PropertyToID("_BloomIntensity");

        static readonly ProfilingSampler sampler = new("Bloom");
        readonly TextureHandle[] pyramid = new TextureHandle[2 * maxBloomPyramidLevels + 1];
        ...
        void Render(RenderGraphContext context)
        {
            ...
            buffer.SetGlobalFloat(bicubicUpsamplingId,
            ...
            if (bloom.mode == PostFXSettings.BloomSettings.Mode.Additive)
            {
                ...
                buffer.SetGlobalFloat(intensityId, 1.0f);
                finalIntensity = bloom.intensity;
            }
            else
            {
                combinePass = finalPass = Pass.BloomScatter;
        ...
        {
            var bloom = stack.Settings.Bloom;
            var size = bloom.ignoreRenderScale
            ...
            using var builder = renderGraph.AddRenderPass(sampler.name, out BloomPass pass, sampler);
            pass.stack = stack;
            ...
            {
                if( size.x < bloom.downscaleLimit || size.y < bloom.downscaleLimit)
                {
                    break;
                }
                desc.width = size.x;
                ...
                pyramid[pyramidIndex + 0] = builder.CreateTransientTexture(desc);
